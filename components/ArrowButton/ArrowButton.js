...
const LinkComponent = ({link, type="external", children, size="small"}) => {
    ...
    if (type == "external") {
        ...
        return <a className={linkStyles} href={`mailto:${link}`}>{size == "small" ? <h6>Email →</h6> : <h2>Email <Arrow /></h2>}</a>
    } else {
        return
    }
}

const ArrowButton = ({text, link, type="external", size="small", leftAlign=false}) => {
    return (
